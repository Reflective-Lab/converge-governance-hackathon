...
This file is the Claude entrypoint. It also contains the shared project philosophy, stack, rules, and Converge guidance that other agents may need. Long-form documentation lives in `kb/`.
...
Run `/focus` at session start, `/checkpoint` at session end. See `kb/Workflow/Daily Journey.md` for the full cheat sheet.

| Skill | Purpose |
|---|---|
| `/focus` | Session opener — orient yourself, see team activity |
| `/sync` | Team sync — who did what, PRs waiting, unclaimed issues |
...
| `/status` | Build health, test results |
