...
2. Validate it locally with `axiom-truth`.
...
The current Tauri command validates specs offline-first using Axiom truth tooling. Business-sense and compilability checks are intentionally disabled until a fuller live validator is added on top of the same `ChatBackend` contract.
